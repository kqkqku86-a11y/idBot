...
bool MacroEditLayer::init() {
    ...
    hoveredBg = NineSlice::create("idbot_bg.png", {0, 0, 80, 80});
    ...
    selectedBg = NineSlice::create("idbot_bg.png", {0, 0, 80, 80});
    ...
    listBg = NineSlice::create("idbot_bg.png", {0, 0, 80, 80});
    ...
    selectedInputBg = NineSlice::create("idbot_bg.png", {0, 0, 80, 80});
...
void MacroEditLayer::onSave(CCObject *) {
    std::string extension =
        Bot::get().replay.frameFixes.empty() || inputs.empty()
            ? "."
    ...
    geode::createQuickPopup(
        ...
        "Cancel", "Yes", [this](auto, bool btn2) {
            ...
            if (auto* popup = MacroEditLayer::create()) {
...
void MacroEditLayer::onMerge(CCObject *) {
    ...
    if (Bot::get().layer) {
        layer = typeinfo_cast<geode::Popup *>(Bot::get().layer);
    } else {
        auto children = CCScene::get()->getChildrenExt<CCNode *>();
    ...
    if (auto* popup = LoadMacroLayer::create(layer, this, false)) {
        popup->m_noElasticity = true;
        popup->show();
    }
}
