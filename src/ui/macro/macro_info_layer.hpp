...
class MacroInfoLayer : public geode::Popup {
  ...
  private:
    bool init() {
        ...
        for (const auto &input : bot.replay.inputs)
        ...
        lbl = CCLabelBMFont::create(bot.replay.author.c_str(), "chatFont.fnt");
        lbl->setScale(0.7f);
        ...
        MDTextArea *desc = MDTextArea::create(
            (bot.replay.description == "" ? "N/A" : bot.replay.description).c_str(),
            {140, 30});
        ...
        lbl = CCLabelBMFont::create(
            geode::utils::numToString(bot.replay.framerate).c_str(),
            "chatFont.fnt");
        lbl->setAnchorPoint({0, 0.5});
        ...
        std::string duration =
            bot.replay.inputs.empty()
                ? std::string("0s")
                : (geode::utils::numToString(bot.replay.inputs.back().frame /
                                             bot.replay.framerate) +
                   "s");
        ...
        std::string version =
            bot.replay.gameVersion != 0
                ? fmt::format("{:g}",
                              static_cast<double>(bot.replay.gameVersion) / 1000.0)
                : "N/A";
        ...
        std::string seed = bot.replay.seed != 0
                               ? geode::utils::numToString(bot.replay.seed)
                               : std::string("N/A");
        ...
        lbl = CCLabelBMFont::create(bot.replay.ldm ? "true" : "false",
                                    "chatFont.fnt");
        lbl->setAnchorPoint({0, 0.5});
        lbl->setScale(0.475f);
        ...
        std::string levelName =
            bot.replay.levelInfo.name != "" ? bot.replay.levelInfo.name : "N/A";
        std::string levelID =
            bot.replay.levelInfo.id != 0
                ? geode::utils::numToString(bot.replay.levelInfo.id)
                : "";
        ...
        lbl = CCLabelBMFont::create(
            (bot.replay.botInfo.name + " " + bot.replay.getBotVersionString()).c_str(),
            "chatFont.fnt");
        lbl->setAnchorPoint({0, 0.5});
        ...
        lbl = CCLabelBMFont::create(
            geode::utils::numToString(bot.replay.inputs.size()).c_str(),
            "chatFont.fnt");
        lbl->setAnchorPoint({0, 0.5});
        lbl->setScale(0.55f);
