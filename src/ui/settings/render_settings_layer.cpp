...
void RenderSettingsLayer::onDefaults(CCObject*) {
    geode::createQuickPopup(
        ...
        [this](auto, bool btn2) {
            ...
            bot.mod->setSavedValue("render_music_volume", 1.0);
            bot.mod->setSavedValue("render_sfx_volume", 1.0);
            bot.mod->setSavedValue("render_file_extension", std::string(".mp4"));
            bot.mod->setSavedValue("render_fade_in", false);
            bot.mod->setSavedValue("render_fade_out", false);
            bot.mod->setSavedValue("render_fade_in_time", geode::utils::numToString(2));
            bot.mod->setSavedValue("render_fade_out_time", geode::utils::numToString(2));
            bot.mod->setSavedValue("render_hide_endscreen", false);
            bot.mod->setSavedValue("render_hide_levelcomplete", false);
            ...
            this->keyBackClicked();
            static_cast<RecordLayer*>(Bot::get().layer)->openMenu(true);
...
bool RenderSettingsLayer::init() {
    ...
    CCMenuItemToggler* toggle =
        CCMenuItemExt::createTogglerWithStandardSprites(0.555f, [this](CCMenuItemToggler* sender) {
            static_cast<RecordLayer*>(Bot::get().layer)->toggleSetting(sender);
        });
    ...
    sfxSlider = SliderNode::create([this](SliderNode*, float) { onSlider(nullptr); });
    sfxSlider->setPosition({188, 24});
    sfxSlider->setAnchorPoint({0.5f, 0.5f});
    sfxSlider->setScale(0.545f);
    sfxSlider->setMin(0.f);
    sfxSlider->setMax(1.f);
    ...
    musicSlider = SliderNode::create([this](SliderNode*, float) { onSlider(nullptr); });
    musicSlider->setPosition({188, 69});
    musicSlider->setAnchorPoint({0.5f, 0.5f});
    musicSlider->setScale(0.545f);
    musicSlider->setMin(0.f);
    musicSlider->setMax(1.f);
    ...
    if (usingApi) {
        ...
        sfxSlider->getBar()->setOpacity(100);
        sfxSlider->getGroove()->setOpacity(100);
        sfxSlider->getThumb()->setOpacity(100);
        musicSlider->getBar()->setOpacity(100);
        musicSlider->getGroove()->setOpacity(100);
        musicSlider->getThumb()->setOpacity(100);
    }
#endif
...
void RenderSettingsLayer::onSlider(CCObject*) {
  if (!sfxSlider || !musicSlider) return; 
