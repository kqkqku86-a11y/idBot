...
class AutoSaveLayer : public geode::Popup, public TextInputDelegate {
  ...
  private:
    ...
    void textChanged(CCTextInputNode *) override {
        ...
        Bot::get().autosaveInterval = static_cast<int>(mins * 60);
    ...
    void onToggle(CCObject *) {
        Loader::get()->queueInMainThread([this] {
            auto &bot = Bot::get();
            bot.autosaveIntervalEnabled = intervalToggle->isToggled();
            bot.mod->setSavedValue("autosave_interval_enabled",
                                 bot.autosaveIntervalEnabled);
            bot.mod->setSavedValue("autosave_checkpoint_enabled",
                                 checkpointToggle->isToggled());
            bot.mod->setSavedValue("autosave_levelend_enabled",
    ...
    void updateInputs() {
        ...
        intervalLbl->setOpacity(bot.autosaveIntervalEnabled ? 255 : 100);
        intervalLbl2->setOpacity(bot.autosaveIntervalEnabled ? 255 : 100);
        Utils::setTextInputEnabled(intervalInput, bot.autosaveIntervalEnabled);
    }

  public:
    static AutoSaveLayer* create() {
        auto* layer = new AutoSaveLayer();
        if (layer && layer->init()) {
            layer->autorelease();
            return layer;
        }
